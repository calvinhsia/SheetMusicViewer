...
MuseScore Studio is started from which you can view, edit or play back your music. You can then drive MIDI output device or export MIDI to play a MIDI device.
...
Conversion of PDF takes a few seconds per page.
The quality and format of the PDF make a huge difference in the accuracy. A scanned image is just an array of pixels, like a photo. 
A PDF generated by music software is digital instructions (like draw lines, symbols, use a font)
You can run Audiveris by itself and input a PDF and Export Book to create an mxl file to see how well it can read the music.
The log output shows the location of temporary intermediate files (like log, mxl, PDF,mxl,omr files).
You can rename the .mxl or the .omr file to .zip and open them to inspect the MusicXML and OMR debug files that Audiveris produces.
